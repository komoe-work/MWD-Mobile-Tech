export async function onRequestPost(context) {
  try {
    ...
    const specs = formData.get('specs'); // Now grabs specs from form
    ...
    await context.env.DB.prepare(
      "INSERT INTO inventory (brand, model, price, specs, image_url) VALUES (?, ?, ?, ?, ?)"
    ).bind(brand, model, price, specs, publicUrl).run();
